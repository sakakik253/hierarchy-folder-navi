...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    Title="フォルダナビゲーター v3.7 - Phase 3.7" Width="1400" Height="800"
        ...
        <Grid Grid.Row="2">
            ...
            <Border Grid.Column="2" BorderBrush="#bdc3c7" BorderThickness="1" Margin="5">
                <Grid>
                    ...
                    <!-- 展開/折りたたみボタン -->
                    <StackPanel Grid.Row="1" Orientation="Horizontal" Margin="5" Background="#ecf0f1">
                        <Button Name="btnExpandAll" Content="▼ すべて展開" Padding="5"
                               Margin="5,2" MinWidth="100" FontSize="10"/>
                        <Button Name="btnCollapseAll" Content="▲ すべて折りたたみ" Padding="5"
                               Margin="5,2" MinWidth="100" FontSize="10"/>
                    </StackPanel>
...
$btnExpandAll = $mainWindow.FindName("btnExpandAll")
$btnCollapseAll = $mainWindow.FindName("btnCollapseAll")

# ===============================================
# TreeView展開/折りたたみ機能
# ===============================================

# すべてのTreeViewItemを再帰的に展開
function Expand-AllTreeViewItems {
    param([System.Windows.Controls.ItemsControl]$parentItem)

    foreach ($item in $parentItem.Items) {
        if ($item -is [System.Windows.Controls.TreeViewItem]) {
            # 遅延読み込み対応: ダミーノードがある場合は実際のフォルダを読み込む
            if ($item.Items.Count -eq 1 -and $item.Items[0] -is [System.Windows.Controls.TreeViewItem] -and $item.Items[0].Header -eq "読み込み中...") {
                $folderPath = $item.Tag
                if ($folderPath -and (Test-Path $folderPath)) {
                    ...
                    try {
                        ...
                        foreach ($subFolder in $subFolders) {
                            ...
                            $childNode.Header = $subFolder.Name
                            $childNode.Tag = $subFolder.FullName

                            # さらに子フォルダがあるか確認
                            if ((Get-ChildItem -Path $subFolder.FullName -Directory -ErrorAction SilentlyContinue | Select-Object -First 1)) {
                                $dummy = New-Object System.Windows.Controls.TreeViewItem
                                $dummy.Header = "読み込み中..."
                                $childNode.Items.Add($dummy) | Out-Null
                            }
            ...
            # 子ノードも再帰的に展開
            Expand-AllTreeViewItems $item
        }
    }
}

# すべてのTreeViewItemを再帰的に折りたたみ
function Collapse-AllTreeViewItems {
    ...
    foreach ($item in $parentItem.Items) {
        if ($item -is [System.Windows.Controls.TreeViewItem]) {
            # 子ノードを先に折りたたむ
            Collapse-AllTreeViewItems $item
...
$btnExpandAll.Add_Click({
    if ($treeView.Items.Count -gt 0) {
        Write-Host "TreeViewをすべて展開します..." -ForegroundColor Yellow
        $txtStatus.Text = "ツリーを展開中..."
        $mainWindow.Cursor = [System.Windows.Input.Cursors]::Wait

        try {
            Expand-AllTreeViewItems $treeView
            $txtStatus.Text = "すべて展開しました"
        }
        catch {
            Write-Host "展開エラー: $_" -ForegroundColor Red
            $txtStatus.Text = "展開中にエラーが発生しました"
        }
        finally {
...
$btnCollapseAll.Add_Click({
    if ($treeView.Items.Count -gt 0) {
        Write-Host "TreeViewをすべて折りたたみます..." -ForegroundColor Yellow

        try {
            Collapse-AllTreeViewItems $treeView
            $txtStatus.Text = "すべて折りたたみました"
        }
        catch {
            Write-Host "折りたたみエラー: $_" -ForegroundColor Red
            $txtStatus.Text = "折りたたみ中にエラーが発生しました"
        }
    }
    else {
        [System.Windows.MessageBox]::Show(
